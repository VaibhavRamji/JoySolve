class FVM:
    ...
    @staticmethod
    def fluxes(rho, u_face, v_face, w_face, dx, dy, dz):
    ...
    def diffusion(
        ...
        Diffu = nu * (
            (u_x_plus - 2 * u_p + u_x_minus) / dx**2
            + (u_y_plus - 2 * u_p + u_y_minus) / dy**2
            + (u_z_plus - 2 * u_p + u_z_minus) / dz**2
        )

        Diffv = nu * (
            (v_x_plus - 2 * v_p + v_x_minus) / dx**2
            + (v_y_plus - 2 * v_p + v_y_minus) / dy**2
            + (v_z_plus - 2 * v_p + v_z_minus) / dz**2
        )

        Diffw = nu * (
            (w_x_plus - 2 * w_p + w_x_minus) / dx**2
            + (w_y_plus - 2 * w_p + w_y_minus) / dy**2
            + (w_z_plus - 2 * w_p + w_z_minus) / dz**2
        )
